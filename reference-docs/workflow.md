...
- **Status:** Built, polished and independently reviewed (approved with notes; all fix-now findings resolved) — **awaiting the owner's manual test and go-ahead** (branch `phase-5-led-structure-and-mapping`, not yet merged)
...
- **QA Test Case doc:** `docs/QA_Desktop_Phase5_LEDStructureAndMapping.md` — 39 cases: 23 passed (automated, rendered, real-Windows), 16 manual (TC-M01 – TC-M16) awaiting the owner; includes the step-by-step real-app guide.
...
- **Deviations / owner decisions:**
  ...
  2. **No shared-folder credentials are stored or asked for** — the app uses the Windows account it runs under. **The venue's authentication method is still an OPEN BRD §36 item** (QA F-33) and must be decided before Phase 8.
  ...
  5. Carried: F-32 (test proves this Windows account only; unattended runs in Phase 12), F-34 (probe visible to a device watching the folder), F-35 – F-37 (see QA doc).
