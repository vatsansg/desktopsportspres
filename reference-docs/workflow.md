...
- **What was built:**
  ...
  - 1107 automated tests pass (Phases 0–6; 263 new) plus 9 live.
- **QA Test Case doc:** `docs/QA_Desktop_Phase6_ChangeLog.md` — 43 cases: 20 passed (automated, live, rendered), 23 manual (TC-M01 – TC-M23) awaiting the owner; includes the step-by-step real-app guide and a dev script (`scripts/insert_test_history.py`) to see "already processed / updated / removed" on the real event.
...
- **Deviations / owner decisions:**
  ...
  6. Carried: F-49 (flat RPI folder shared by all events), F-50 (streaming for large files, Phase 7), F-43 (local vs UTC time in the CSV), F-44 (result kept in memory only), **F-46 (Phase 7 must re-validate every name at the write site, refuse links, stay inside the event folder, and resolve real blob names case-sensitively)**.
