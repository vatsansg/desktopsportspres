...
| D5a (added) | **Downloads reach disk, deliberately and only through the operator's own choice** | **Pass (after an owner-reported fix)** | `pywebview` refuses every download by default; WebView2 cancelled the CSV export silently (F-67). Downloads are now allowed, but this is not a general write capability handed to page content: WebView2's own native **Save As** dialog decides the name and folder for every download, defaulting to the operator's Downloads folder — the application still writes nothing to disk itself. The only thing on the page that can trigger a download is the Logs page's own `Export CSV` link, which the operator must be signed in to reach; there is no other file link, upload target, or externally-reachable content in the application that a download could be triggered from. |
