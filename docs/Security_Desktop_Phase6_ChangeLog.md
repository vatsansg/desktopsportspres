...
| D12 (added) | **Local writes are confined to one dedicated folder** | **Pass (pending review)** | `services/localfiles.py`: the folder cannot be an operating-system folder or the application's data folder (a file named `ledsync.db` can never replace the database); a name is one plain, Windows-safe file name (no `:`/stream, device name, trailing dot or space, separator); the target must be a plain file or absent — a folder, symbolic link or **junction** with that name is never replaced, followed or deleted (tested with a real junction); data goes to a temporary file first and is moved into place, so a failed write leaves no half file and no temp file; only the file's own name is ever deleted. |
| D13 (added) | **The RPI folder setting cannot point somewhere dangerous** | **Pass (pending review)** | `settings.py` owns the `rpi_folder` key (scope guard extended; other keys still refused). A chosen folder must be an absolute local or network path, not an operating-system folder, not inside the application data folder, no `..`, streams, device names or invisible characters (same rules as device folders, 8 unsafe forms tested). The default is `<data folder>\RPI`. |
| D14 (added) | **Real blob names are looked up, not guessed** | **Pass (pending review)** | The log says `RPI/…`, Azure holds `rpi/…` (case-sensitive). `resolve_relative_path` lists each level with the existing read-only call, ignores A–Z case only, refuses two matches, and refuses a path with `..`/unsafe characters before any call. The AST guards that forbid write operations still pass (two harmless string helpers were added to their allow-list). |
