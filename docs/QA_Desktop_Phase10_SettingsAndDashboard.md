...
|---|---|---|---|---|
| 38 | 17 (automated groups covering 41 new pytest cases, 1 live check against real Azure, 1 rendered-page group — all run by Claude) | 0 | 0 | 22 (manual TC-M01 – TC-M22, for the owner) |

`.\.venv\Scripts\python -m pytest -q` → **1442 passed, 14 skipped** (the skipped are the live-Azure tests, which need `--live`).
