...
|---|---|---|---|---|
| 39 | 39 — 18 automated groups (covering 44 new pytest cases, 1 live check against real Azure, 1 rendered-page group, run by Claude) + 22 manual TC-M01 – TC-M22, run by the owner in the real app ("All good" 23/09/26, one finding — see F-71) | 0 | 0 | 0 |

`.\.venv\Scripts\python -m pytest -q` → **1443 passed, 14 skipped** (the skipped are the live-Azure tests, which need `--live`).
...
| TC-A14 | **Second-review regression test:** a held file goes through the real Download & Sync path end to end (not just the comparison layer) and gets **no row at all** in `download_history` — not Failed, not Skipped, nothing — and never reaches its mapped LED device; turning the cut-off off (or, in real life, the boundary passing) picks it up cleanly on the very next run, exactly once | Pass |
...
| TC-A15 | **Boundary-status line (owner-reported fix, 23/09/26):** absent when no cut-off is currently active (never saved, or saved but the "Enable cut-off" switch is off - the emergency override); present and equal to the server's own `cutoff_boundary()` instant, in the exact `data-utc-instant-preview` attribute the page's JS reads, whenever a cut-off is enabled with a saved time | Pass |
...
| F-71 | **Closed 23/09/26 (owner-reported).** The local-time preview's fixed "today" wording didn't just mis-state DST — it mis-stated which calendar day the cut-off is actually holding back to on *every single day*, for any timezone offset from UTC. The owner found it live: at 07:45 SGT (UTC+8) with a `21:00` SGT cut-off, the boundary the app actually uses is already *yesterday's* 21:00 (UTC calendar "today" has already passed the cut-off hour even though local calendar "today" hasn't reached it yet), but the preview said "= 21:00 local time **today**," reading as a future event rather than an already-active boundary. **Fixed:** the page now renders a second line computed from the server's own `changes.cutoff_boundary()` — the exact function the real held/eligible decision uses, never re-derived in JS — stating the actual boundary and the next one with dates: "Right now, changes are held back if made after 22/09/26 21:00 local time. The next quiet period begins 23/09/26 21:00 local time." The original per-keystroke preview is kept for converting the typed clock reading, reworded to drop the misleading "today" and switched to 24-hour format for house-style consistency (TC-A15). | Info | Fixed |
...
| Independent Solution Architect review (recurring cut-off redesign) | Independent review agent (fresh context) | 22/09/26 | **Approved, no findings** — the single most important property (a held file can never become permanently stuck) was verified by an executable probe tracing the full pipeline: a held assessment carries a distinct action value that is structurally filtered out before `assets.asset_items()`/`rpi`'s equivalent hand any work to the download engine, so nothing is ever written to `download_history` for it, and `sync.plan()` (the push engine) only reads rows that exist — a held deletion therefore cannot reach a device either. The boundary math, the full held/eligible truth table (New/Updated/Deleted, exactly-on-the-boundary inclusive), the emergency-override bypass, CSRF/validation on the web form, the CSP-safe JS, and the two-column schema upgrade (including a simulated partially-upgraded database) were all verified sound. One recommendation — a permanent end-to-end regression test proving the no-stuck-file property, since the existing tests stopped at the comparison layer — has been added (TC-A14). Two cosmetic notes, both accepted, not fixed: an unusual edge case where an Azure-only file with a locally-deleted record shows the reason "Already downloaded" instead of something more specific; and unchecking Enable while the time box holds an unsaved malformed value blocks the save (fails safe, keeps the previous setting) rather than allowing the switch alone to turn off. |
| User (Vatsan) go-ahead | Vatsan | 23/09/26 | **"All good"**, with one finding from real use: the cut-off's local-time preview said "today" in a way that mis-stated which day was actually being held back at certain times of day (SGT early morning, `21:00` SGT cut-off) — see F-71. Fixed (a server-computed boundary-status line replaces the guess) and re-tested by Claude before this go-ahead; not put back through a second owner or architect pass. |
