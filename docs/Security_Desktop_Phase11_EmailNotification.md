...
| S-69 | **Open, Medium — carried to Phase 12** | (Owner-reported, 23/09/26) In the owner's own venue-machine environment, the notification failed with `Notification failed: An unexpected problem occurred sending the notification (Error).` - an exception whose type name is literally `Error`. Not reproduced on the development machine (the same real credentials sent successfully there, both standalone and through a full real Download & Sync); an exhaustive check of the installed `azure-core`/`azure-communication-email` source found no exception class named bare `Error` in any code path this module can reach, so the root cause is not yet identified. Critically, `_send`/`notify()`'s own exception handling worked exactly as designed throughout: the failure was caught, classified as best it could be, logged, and never affected the underlying Download & Sync operation, which completed and logged normally either way (D3 above) - this is a notification-accuracy gap, not a run-safety one. The full traceback (already captured by `log.exception()` in `<data dir>\logs\ledsync.log`) has not yet been retrieved from that machine. | Not fixed - to be root-caused when Phase 12 (scheduled runs, which depend on the same notifier) begins. |
...
| User (Vatsan) go-ahead | Vatsan | 23/09/26 | **Go-ahead given.** Live validation complete against the owner's real Azure Communication Services resource; a new sender identity ("WTT Desktop Asset Management") was created on the same verified domain the web application already uses, and the owner confirmed real email receipt. A separate failure with an unidentified `(Error)` exception was found afterwards in the owner's own environment (S-69) - does not affect the underlying Download & Sync operation's safety or correctness; owner asked to proceed and revisit as part of Phase 12. |
