...
| F-76 | **Open, carried to Phase 12.** In the owner's own venue-machine environment (not this development machine), a Download & Sync completed successfully but the notification logged `Notification failed: An unexpected problem occurred sending the notification (Error).` — an exception literally named `Error` at the type-name level. It is reproducible with the same real credentials in this environment (a live send there succeeds cleanly, and the exhaustive Azure SDK source checks done here found no exception class named bare `Error` in any reachable code path), so the difference is specific to the owner's machine/environment and not yet root-caused. The full traceback (written to `<data dir>\logs\ledsync.log` by `log.exception(...)`, which this fix already added) was not yet retrieved. **This never affected the Download & Sync operation itself** — every run has completed and logged correctly regardless of the notification outcome, exactly as the "never breaks the run" guarantee requires (see D3, Security doc) — so it does not block using the feature; it blocks trusting that notifications reliably arrive in that environment until root-caused. To be investigated when Phase 12 (scheduled runs, which will also depend on this same notifier) begins. | Medium | **Open — carried to Phase 12** |
...
| User (Vatsan) go-ahead | Vatsan | 23/09/26 | **Go-ahead given** — live validation complete and email receipt confirmed on the development side (23/09/26). A separate reproduction of the notification failing with an unidentified exception in the owner's own venue-machine environment (F-76) was found afterwards; the owner asked to proceed and revisit it as part of Phase 12, since it does not affect the Download & Sync operation itself. |
