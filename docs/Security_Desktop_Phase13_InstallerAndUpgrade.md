...
| User (Vatsan) go-ahead | Vatsan | 24/09/26 | **Go-ahead given.** Owner installed and validated the compiled installer on their own machine and uploaded it plus the config template to the real `desktopinstaller` container themselves. F-86/F-87 (QA doc) remain recommended follow-ups for the IT operations team's first real rollout, not blockers. |
| — | — | 24/09/26 | Caught during this same session, before any commit: the tracked `installer/install-config.example.json` had been edited in place with two real secrets (a live Storage Account key, a live ACS connection string key) while the owner was testing locally. Reverted before staging (never committed - confirmed via `git diff`/history); `.gitignore` extended so any real `install-config*.json` other than the example file is now ignored outright, preventing a repeat. |
