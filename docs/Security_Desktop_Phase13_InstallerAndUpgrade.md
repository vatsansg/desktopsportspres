...
| # | Item | Status | Notes |
|---|---|---|---|
| H1 | New dependency reviewed | **Pass** | `pyinstaller==6.22.3` (build-only, `requirements-build.txt` - never installed on a venue machine, never part of the shipped application's own runtime closure). Its own transitive build-time dependencies (`pefile`, `pyinstaller-hooks-contrib`, `pywin32-ctypes`, `altgraph`) are likewise build-only. `pip-audit` against `requirements.txt` (the actual shipped runtime closure) is unchanged from Phase 11. |
...
|---|---|---|---|
| Independent Solution Architect review | — | — | Pending — covered by the all-phases review requested ahead of hand-off, rather than a Phase-13-only pass (owner instruction, 23 Sep 2026). |
| User (Vatsan) go-ahead | Vatsan | pending | pending — needs the Inno Setup packaging + clean-machine/upgrade validation (QA doc F-82) and the all-phases review |
