...
from ..services import changes, connectivity, eventstatus, exceptions, events as event_service, mappings, registration as reg, structure
...
@bp.post("/<event_id>/cutoff")
@login_required
def save_cutoff(event_id):
    row = _load(event_id)
    db = get_db()
    try:
